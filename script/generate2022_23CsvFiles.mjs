/**
 * Generate Complete .CSV Backup Files for Financial Year 2022-23
 * Output Directory: script/data/backups_2022_23/
 *
 * Generated CSVs:
 * 1. customers.csv
 * 2. sales.csv (Item = 'Others', Category = 'Sales')
 * 3. purchases.csv (Item = 'Others', Category = 'Purchase', 100% Cash)
 * 4. expenses.csv (ExpenseCategory mapped to modal: Fuel, Interest, Labor, Tools, Others)
 * 5. services.csv (Item = 'Pickup' for Daalu, 'Brokerage' for commission)
 * 6. payments.csv (Recorded from Credit List C1/C2/C3 and Kasar)
 * 7. monthly_rollout.csv (Monthly Trading Summaries & P&L)
 */
...
// 1. Load Alias Dictionary and Existing Customers
...
const customerMap = new Map(); // norm -> { id, canonicalName, creditLimit, outstandingAmount }
...
// ── Step 1: Read Customer Credit List (Year-End 2022-23) ───────────────
...
for (let i = 1; i < cRows1.length; i++) {
  ...
  if (c1 > 0) {
    payments2022_23.push({
      PaymentID: `pmt_2023_03_c1_${i}`,
      CustomerID: cust.id,
      CustomerName: cust.canonicalName,
      Date: '2023-03-31T18:30:00.000Z',
      AmountPaid: c1,
      Notes: `Payment C1 in Customer Credit List`,
    });
  }
  if (c2 > 0) {
    payments2022_23.push({
      PaymentID: `pmt_2023_03_c2_${i}`,
      CustomerID: cust.id,
      CustomerName: cust.canonicalName,
      Date: '2023-03-31T18:30:00.000Z',
      AmountPaid: c2,
      Notes: `Payment C2 in Customer Credit List`,
    });
  }
  if (c3 > 0) {
    payments2022_23.push({
      PaymentID: `pmt_2023_03_c3_${i}`,
      CustomerID: cust.id,
      CustomerName: cust.canonicalName,
      Date: '2023-03-31T18:30:00.000Z',
      AmountPaid: c3,
      Notes: `Payment C3 in Customer Credit List`,
    });
  }
  if (kasar > 0) {
    payments2022_23.push({
      PaymentID: `pmt_2023_03_kasar_${i}`,
      CustomerID: cust.id,
      CustomerName: cust.canonicalName,
      Date: '2023-03-31T18:30:00.000Z',
      AmountPaid: kasar,
...
for (let i = 1; i < cRows3.length; i++) {
  ...
  cust.outstandingAmount = (cust.outstandingAmount || 0) + total;
}

console.log(`✅ Loaded ${customerMap.size} customers from Credit List`);
...
for (const m of MONTHS) {
  ...
  for (let i = 1; i < endIdx; i++) {
    ...
    // Brokerage handling (service income)
    if (
      ...
      services2022_23.push({
        ...
        Item: 'Brokerage',
        Amount: bAmt,
        Notes: `Brokerage service income from ${m.key}`,
      });
      continue;
    }
    ...
    // Grass Sale (Item='Others' as requested)
    sales2022_23.push({
      TransactionID: `sale_${m.period}_${i}`,
      CustomerID: cust.id,
      CustomerName: cust.canonicalName,
      Date: dateStr,
      Item: 'Others', // Set to Others as requested
      WeightKg: kg,
      Rate: rate,
    ...
    // Pickup Service (Item='Pickup' as requested)
    if (daaluAmt > 0) {
      services2022_23.push({
  ...
  // ── D. Monthly Rollout Data (Main Tab & Opening/Closing Tab) ───────
  ...
  // Read Opening/Closing Tab for detailed cash and capital balances
...
console.log('\n💾 Step 3: Writing all 2022-23 .CSV Files...');
...
console.log(
  `✅ payments.csv: ${pmtRowsOut.length - 1} records (C1/C2/C3 & Kasar)`,
);
...
const metaRowsOut = [
  metaHeader,
  `financial_year,2022-23,${new Date().toISOString()},Generated normalized 2022-23 dataset`,
...
console.log(`\n🎉 ALL 8 CSV FILES SUCCESSFULLY GENERATED IN: ${OUT_DIR}`);
